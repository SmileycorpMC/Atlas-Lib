...
import net.smileycorp.atlas.api.block.MultifaceBlock;
import org.apache.commons.lang3.ArrayUtils;

public class ItemMultifaceBlock<T extends Block & MultifaceBlock<T>> extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (block.multipartClass() == state.getBlock().getClass()) {
            if (MultifaceBlock.hasFacing(state, facing)) return EnumActionResult.PASS;
            state = block.getBlockStateFromFacing(ArrayUtils.add(MultifaceBlock.getFacings(state), facing));
