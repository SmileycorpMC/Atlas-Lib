...
public interface MultifaceBlock<T extends Block & MultifaceBlock> {
    ...
    static int getMeta(IBlockState state) {
        if (!(state.getBlock() instanceof MultifaceBlock)) return 0;
        return ((MultifaceBlock) state.getBlock()).getOrdinal() * 16 + state.getValue(META);
    }
    
    static <T extends Block & MultifaceBlock> List<T> create(Function<Integer, T> instance) {
        List<T> blocks = Lists.newArrayList();
        for (int i = 0; i < 4; i++) blocks.add(instance.apply(i));
        return blocks;
    }

    enum EnumMultifaceDirection {
